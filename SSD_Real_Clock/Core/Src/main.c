...
uint8_t digit[]={0x3F,0x06,0x5B,0x4F,0x66,0x6D,0x7C,0x07,0x7F,0x6F,0x80};
uint8_t ssd[4];
volatile uint16_t hour=00;
volatile uint16_t min=00;
...
while (1)
{
	...
	for(int i=0;i<4;i++)
	{
		...
		HAL_GPIO_WritePin(GPIOA,ssd[i] | (~(1<<i)<<8),GPIO_PIN_SET);
		HAL_Delay(10);
	}
    /* USER CODE END WHILE */
